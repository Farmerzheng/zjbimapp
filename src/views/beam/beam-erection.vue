<template>
  <layout title="架梁分析" :left-arrow="true">
    <div>
...
import { beamErection, beamType } from '@/api/beam'
...
export default defineComponent({
  ...
  setup() {
    ...
    //
    const getBeamErection = async (params) => {
      let res = await beamErection(params)
      if (res.code !== 0) {
        Toast.fail(res.msg)
        return
      }
    ...
    const initMonth = async () => {
      ...
      let data = await getBeamErection(state.params)
      if (data.length == 0) {
